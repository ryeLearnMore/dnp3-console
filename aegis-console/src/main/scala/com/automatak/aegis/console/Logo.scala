...
object Logo {
  ...
  private val copyright = "DNP3 Console - Automatak, LLC - Licensed under the terms of Apache v2"

  def printLine(fg: Ansi.Color)(msg: => String): Unit =
    AnsiConsole.out.println(Ansi.ansi().fg(fg).a(msg).reset())

  def print(): Unit = {
    println("")
    printLine(WHITE)(copyright)
    println("")
  }
}
